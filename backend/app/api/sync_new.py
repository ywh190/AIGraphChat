...
def progress_callback(progress_data: dict):
    ...
    try:
        ...
        # 确保存储的数据包含正确的 in_progress 和 status 字段
        if isinstance(progress_data, dict):
            current_step = progress_data.get('current_step', '')
            status = progress_data.get('status', '')
            
            # 根据状态设置 in_progress
            if current_step == 'completed' or status == 'completed':
                progress_data['status'] = 'completed'
                progress_data['in_progress'] = False
                # 同步完成后清除 Redis 数据
                redis_client.delete('sync_progress')
                print("[SYNC] 同步完成，已清除进度缓存")
            elif current_step in ['failed', 'error'] or status in ['failed', 'error']:
                progress_data['status'] = 'failed'
                progress_data['in_progress'] = False
                # 失败时也清除缓存
                redis_client.delete('sync_progress')
                print("[SYNC] 同步失败，已清除进度缓存")
            else:
                # 正在同步中
                progress_data['in_progress'] = status in ['running', 'RUNNING']
                # 存储到 Redis
                redis_client.set('sync_progress', progress_data, ttl=3600)

        # 同时更新内存变量作为备份
        with _progress_lock:
            _sync_progress.update(progress_data if isinstance(progress_data, dict) else {})
...
async def get_sync_progress():
    ...
    try:
        ...
        if progress_data and isinstance(progress_data, dict):
            current_step = progress_data.get('current_step', '')
            status = progress_data.get('status', '')
            in_progress = progress_data.get('in_progress', False)
            
            print(f"[SYNC DEBUG] Redis进度数据: status={status}, current_step={current_step}, in_progress={in_progress}")
            
            # 只有当状态明确为 running 且 in_progress 为 True 时才返回进度数据
            # 其他任何情况都清除数据并返回 idle
            if status == 'running' and in_progress == True:
                # 确实有正在运行的同步任务
                return {
                    "success": True,
                    "data": progress_data
                ...
                # 不是正在运行的同步任务，清除所有旧数据
                print(f"[SYNC] 清除旧的进度数据（非运行状态）: status={status}, current_step={current_step}")
                _clear_all_progress_data()
                return _get_idle_response()
        else:
            # Redis中没有数据，清除内存变量并返回 idle
            _clear_all_progress_data()
            return _get_idle_response()
...
async def clear_sync_cache():
    """清除同步相关缓存"""
    from app.cache import cache_clear, redis_client
