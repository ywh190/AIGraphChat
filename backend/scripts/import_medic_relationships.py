...
from app.models.models import Base, Medic, Herb, medic_herb_association
from sqlalchemy import text
from sqlalchemy.orm import sessionmaker


def build_medic_herb_relationships():
    """建立中成药-药材关联关系"""
    print("\n" + "=" * 70)
    print("建立中成药-药材关联关系")
    print("=" * 70)
    ...
    try:
        # 清空现有关联关系
        print("\n[1/4] 清空现有关联关系...")
        session.execute(text("DELETE FROM medic_herbs"))
        session.commit()
        print("[OK] 已清空")

        # 查询所有中成药
        print("\n[2/4] 查询中成药数据...")
        medics = session.query(Medic).all()
        print(f"[INFO] 找到 {len(medics)} 条中成药")
        ...
        for i, medic in enumerate(medics, 1):
            try:
                composition = medic.composition or ""
                ...
                for herb_id in herbs_in_composition:
                    insert_stmt = text("""
                        INSERT INTO medic_herbs
                        (medic_id, herb_id, created_at)
                        VALUES (:medic_id, :herb_id, NOW())
                    """)
                ...
                if i % 100 == 0:
                    session.commit()
                    print(f"[PROGRESS] 已处理 {i}/{len(medics)} 条中成药，已建立 {success_count} 个关联")

            except Exception as e:
                error_count += 1
                print(f"[ERROR] 处理中成药 {medic.name} 失败: {e}")
                session.rollback()
        ...
        # 统计结果
        total_relationships = session.execute(text("SELECT COUNT(*) FROM medic_herbs")).scalar()
        print(f"\n[OK] 关联建立完成")
        print(f"  中成药总数: {len(medics)}")
...
if __name__ == "__main__":
    build_medic_herb_relationships()
