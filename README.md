...
| 图谱能力 | 实现状态 | 工程边界 |
| --- | --- | --- |
| 基于共享药材/功效的 Jaccard 相似度 | 已接入后端 API | 属于集合相似度，不是图嵌入或语义推理 |
| 子图扩展与节点查询 | 已接入后端 API | 子图服务当前未完整组装返回的边数组 |
| `shortestPath` 最短路径 | 已实现服务层函数 | 尚未接入当前后端公开路由 |
| 模式搜索 | 已实现基础版 | 当前实质是节点名称 `CONTAINS` 检索，不是通用图模式推理 |
| 规则推理 / 链路预测 | 未实现 | 需要额外规则体系、训练数据与评估方法 |
| Neo4j GDS 中心性/社区发现 | 未实现 | 前端服务层存在相关调用形式，但后端没有对应算法路由 |

这些边界是当前版本的真实工程状态：项目已从单纯可视化向可解释相似度和路径分析延伸，但尚未达到基于图算法或知识规则的完整推理系统。
